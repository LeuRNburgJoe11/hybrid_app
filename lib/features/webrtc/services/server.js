...
app.post('/api/handshake-escalation', async (req, res) => {
  try {
    ...
    if (!cfResponse.data || !cfResponse.data.iceServers) {
      throw new Error("Cloudflare response missing iceServers array.");
    }

    // FIX 3: Extract the TURN server entry (the one with username/credential)
    // Google STUN servers come from Cloudflare too but we only need the TURN entry
    // for Flutter. Flutter adds its own STUN fallbacks.
    const iceServers = cfResponse.data.iceServers;
